...
class C3k2(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        # print(f"c1:{c1} c2:{c2} c3k:{c3k} e:{e} g:{g} n:{n} shortcut:{shortcut}")
...
if __name__ == "__main__":
    ...
    print("C3K2 input shape: ",dummy_input.shape)
    print("C3K2 Output shape: ",dummy_output.shape)

    # Create a dummy input tensor (batch=1, channels=128, height=64, width=64)
    dummy_input = torch.randn(1, 128, 64, 64)
    # Initialize C2PSA
    c2psa = C2PSA(c1=128, c2=128, n=2, e=0.5)
    print(f"Number of parameters: {sum(p.numel() for p in c2psa.parameters()) / 1e6:.2f}M")
    # Forward pass
    output = c2psa(dummy_input)
    print(f"C2PSA Input shape: {dummy_input.shape}")
    print(f"C2PSA Output shape: {output.shape}")

    # Test PSABlock standalone
    psablock = PSABlock(c=64, attn_ratio=0.5, num_heads=4)
    dummy_input2 = torch.randn(1, 64, 32, 32)
    output2 = psablock(dummy_input2)
    print(f"PSABlock input shape: {dummy_input2.shape}")
    print(f"PSABlock output shape: {output2.shape}")
